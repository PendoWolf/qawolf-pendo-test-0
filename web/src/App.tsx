...
export default function App() {
  ...
  const run = async (name: string, fn: () => Promise<AppState>) => {
    ...
    try {
      ...
      const properties: Record<string, string | number> = { counterValue: newState.counter };
      if (name === "increment" || name === "decrement" || name === "reset") {
        properties.previousValue = previousValue;
      }
      if (name === "load") {
        properties.lastAction = newState.lastAction;
      }
